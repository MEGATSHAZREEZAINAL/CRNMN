...
This document provides essential context about the CORNMAN Strategic HQ project for use with Qwen Code, an interactive CLI agent. This is a comprehensive, full-stack business management platform built with React, TypeScript, and Supabase, designed for enterprise use.
...
- **Demo Mode:** The frontend automatically detects if Supabase is configured and runs in a local/demo mode if not, using mock data and local storage.
