...
class APIService {
  ...
  private async getCollection<T>(collectionName: string, offlineGetter: () => Promise<T[]>, q?: any): Promise<T[]> {
    try {
      ...
      if (data) {
        await offlineStorage.saveData(collectionName, data);
      }
  ...
  async getInvoices(): Promise<Invoice[]> {
    ...
    return invoices.map(invoice => ({...invoice, customerName: invoice.customer_name}))
  }
  ...
  async getAIInsights(
    ...
    const insights = await this.getCollection('ai_insights', () => offlineStorage.getAIInsights(limit_val), query(collection(db, 'ai_insights'), orderBy('createdAt', 'desc'), limit(limit_val)));
    return insights.map(insight => ({...insight, timestamp: insight.createdAt.toMillis()}))
  }
  ...
  async getBusinessSettings(): Promise<{
    ...
    try {
      if (!this.isOnline()) {
        const settings = await offlineStorage.getAppData('businessSettings');
        return (
          settings || {
