...
🔥 *Jualan Terkini:*
${latestSale.product} - RM${latestSale.amount.toFixed(2)}

📈 *Ringkasan:*
• Jumlah transaksi: ${totalSales}
• Masa terakhir: ${new Date(latestSale.created_at).toLocaleString('ms-MY')}`;
    } catch (error: unknown) {
  ...
  private async getTodayRevenue(): Promise<string> {
    try {
      ...
      const totalRevenue = todaySales?.reduce((sum, sale) => sum + sale.amount, 0) || 0;
