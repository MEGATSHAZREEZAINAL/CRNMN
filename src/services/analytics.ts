...
// Local analytics shape uses ecommerce Order/Product, and app Customer
...
class AnalyticsService {
  ...
  async getRevenueAnalytics(): Promise<RevenueAnalytics> {
    ...
    try {
      ...
      const analytics = this.calculateRevenueAnalytics(sales as any);
  ...
  async getSalesAnalytics(): Promise<SalesAnalytics> {
    ...
    try {
      const sales = (await api.getSales()) as any[];
      const analytics = this.calculateSalesAnalytics(sales as any);
      this.setCache(cacheKey, analytics);
      return analytics;
    } catch (error) {
  ...
  async getInventoryAnalytics(): Promise<InventoryAnalytics> {
    ...
    try {
      const inventory = (await api.getInventory()) as any[];
      const analytics = this.calculateInventoryAnalytics(inventory as any);
      this.setCache(cacheKey, analytics);
      return analytics;
    } catch (error) {
  ...
  async getCustomerAnalytics(): Promise<CustomerAnalytics> {
    ...
    try {
      ...
      return this.calculateCustomerAnalytics(offlineCustomers as any, offlineSales as any);
    }
  }
  ...
  async getRealTimeMetrics() {
    try {
      ...
      const todaySales = allSales.filter((sale: any) => new Date(sale.createdAt ?? sale.created_at) >= today);
      const monthSales = allSales.filter((sale: any) => new Date(sale.createdAt ?? sale.created_at) >= thisMonth);

      const inventory = await api.getInventory();

      return {
         todayRevenue: todaySales?.reduce((sum, sale: any) => sum + Number(sale.amount ?? sale.total ?? 0), 0) || 0,
         monthRevenue: monthSales?.reduce((sum, sale: any) => sum + Number(sale.amount ?? sale.total ?? 0), 0) || 0,
  ...
  private calculateRevenueAnalytics(sales: Array<Order | any>): RevenueAnalytics {
    const now = new Date();
    ...
    const monthlyRevenue = sales
      .filter((sale) => new Date(sale.createdAt ?? sale.created_at) >= startOfMonth)
      .reduce((sum, sale) => sum + (sale.total ?? sale.amount ?? 0), 0);
    const dailyRevenue = sales
      .filter((sale) => new Date(sale.createdAt ?? sale.created_at) >= startOfDay)
      .reduce((sum, sale) => sum + (sale.total ?? sale.amount ?? 0), 0);

    // Product revenue analysis
    const productRevenue = sales.reduce((acc, sale) => {
      const product = (sale as any).product || (sale.items?.[0]?.name) || 'Unknown';
      acc[product] = (acc[product] || 0) + (sale.total ?? sale.amount ?? 0);
      return acc;
    }, {});
  ...
  private calculateSalesAnalytics(sales: Array<Order | any>): SalesAnalytics {
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate());

    const totalSales = sales.length;
    const monthlySales = sales.filter((sale) => new Date(sale.createdAt ?? sale.created_at) >= startOfMonth).length;
    const dailySales = sales.filter((sale) => new Date(sale.createdAt ?? sale.created_at) >= startOfDay).length;
    ...
    const productSales = sales.reduce((acc, sale) => {
      ...
      acc[product].count += 1;
      acc[product].revenue += (sale.total ?? sale.amount ?? 0);
      return acc;
    }, {});
  ...
  private calculateInventoryAnalytics(inventory: Array<Product | any>): InventoryAnalytics {
    const totalItems = inventory.length;
    const lowStockItems = inventory.filter((item: any) => item.stock <= (item.threshold ?? item.lowStockThreshold ?? 0)).length;
    const outOfStockItems = inventory.filter((item) => item.stock === 0).length;
    const inventoryValue = inventory.reduce(
      (sum, item: any) => sum + item.stock * (Number(item.cost ?? item.cost_price ?? item.price ?? 0)),
      0,
    );
    ...
    return {
      ...
      fastMovingItems: inventory
        .sort((a, b) => (Number((b as any).sales_velocity ?? 0)) - (Number((a as any).sales_velocity ?? 0)))
        .slice(0, 5)
        .map((item) => ({
          item: item.name,
      ...
      slowMovingItems: inventory
        .filter((item: any) => (item.sales_velocity || 0) < 1)
        .map((item) => ({
          item: item.name,
          daysInStock: Math.floor(
           (Date.now() - new Date((item as any).created_at ?? item.createdAt).getTime()) / (1000 * 60 * 60 * 24),
          ),
          stock: item.stock,
        })),
      stockAlerts: inventory
         .filter((item: any) => item.stock <= (item.threshold ?? item.lowStockThreshold ?? 0))
        .map((item) => ({
          item: item.name,
          currentStock: item.stock,
           reorderPoint: (item.threshold ?? item.lowStockThreshold ?? 0),
  ...
  private calculateCustomerAnalytics(customers: Customer[], sales: Order[]): CustomerAnalytics {
    ...
    const newCustomers = customers.filter(
      (customer) => new Date((customer as any).created_at ?? (customer as any).createdAt ?? 0) >= thirtyDaysAgo,
    ).length;
    ...
    const customerSpending = sales.reduce((acc, sale) => {
      ...
      if (!acc[customerId]) {
        acc[customerId] = {
          totalSpent: 0,
          orders: 0,
          lastOrder: (sale as any).created_at ?? sale.createdAt,
        };
      }
      acc[customerId].totalSpent += (sale as any).amount ?? sale.total ?? 0;
      acc[customerId].orders += 1;
      const saleDate = new Date((sale as any).created_at ?? sale.createdAt);
      if (saleDate > new Date(acc[customerId].lastOrder)) {
        acc[customerId].lastOrder = saleDate.toISOString();
      }
      return acc;
    }, {});
  ...
  private calculateGrowthRate(data: Order[], type: 'revenue' | 'count'): number {
    ...
    const recent = data.filter((item) => new Date((item as any).created_at ?? item.createdAt) >= thirtyDaysAgo);
    const previous = data.filter(
      (item) =>
        new Date((item as any).created_at ?? item.createdAt) >= sixtyDaysAgo && new Date((item as any).created_at ?? item.createdAt) < thirtyDaysAgo,
    );

    const recentValue =
      type === 'revenue' ? recent.reduce((sum, item) => sum + (item.total ?? (item as any).amount ?? 0), 0) : recent.length;

    const previousValue =
      type === 'revenue' ? previous.reduce((sum, item) => sum + (item.total ?? (item as any).amount ?? 0), 0) : previous.length;
  ...
  private calculateMonthlyTrend(sales: Order[]) {
    const monthlyData = {};
    sales.forEach((sale) => {
      const month = new Date((sale as any).created_at ?? sale.createdAt).toISOString().substring(0, 7);
      if (!monthlyData[month]) {
        monthlyData[month] = { revenue: 0, orders: 0 };
      }
      monthlyData[month].revenue += (sale.total ?? (sale as any).amount ?? 0);
  ...
  private calculateSalesByCustomer(sales: Array<Order | any>) {
    const customerSales = sales.reduce((acc, sale) => {
      ...
      acc[customerId].totalValue += (sale as any).amount ?? sale.total ?? 0;
      return acc;
    }, {});
  ...
  private calculatePerformanceTrend(sales: Order[]) {
    const dailyData = {};
    sales.forEach((sale) => {
      const date = new Date((sale as any).created_at ?? sale.createdAt).toISOString().split('T')[0];
      if (!dailyData[date]) {
        dailyData[date] = { sales: 0, revenue: 0 };
      }
      dailyData[date].sales += 1;
      dailyData[date].revenue += (sale.total ?? (sale as any).amount ?? 0);
    });
  ...
  private calculateAcquisitionTrend(customers: Customer[]) {
    ...
    customers.forEach((customer) => {
      const month = new Date((customer as any).created_at ?? (customer as any).createdAt ?? 0).toISOString().substring(0, 7);
